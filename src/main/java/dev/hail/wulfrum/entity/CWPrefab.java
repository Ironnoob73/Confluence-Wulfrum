...
public class CWPrefab extends AbstractPrefab {
    public CWPrefab(int health, int armor, int attack, int followRange, float knockBack, float knockbackResistance) {
        ...
        this.SIMPLE_MONSTER.setNavigation((e) -> new FlyingPathNavigation(e, e.level())).setSafeFall(1000.0F).setNoGravity().setPushable(false).setNoFriction().addGoal((g, e) -> {
    ...
    public static Supplier<AbstractMonster.Builder> HOVERCRAFT_BUILDER = () -> (new AbstractPrefab(15, 4, 12, 32, 0.0F, 0.9F)).getPrefab().setHurtSound(TESounds.ROUTINE_HURT).setDeathSound(TESounds.ROUTINE_DEATH).setNoAttachAttack().setMovementSpeed(0.5F).setNoGravity();
}
